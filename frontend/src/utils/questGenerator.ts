import type { Quest, QuestType, Difficulty, QuestLength, QuestGenerationParams } from '../types';
import {
  QUEST_TYPES,
  CREATURES,
  LOCATIONS,
...
export class QuestGenerator {
  ...
  private getCountByDifficulty(difficulty: Difficulty, easy: number, medium: number, hard: number, epic: number): number {
    switch (difficulty) {
  ...
  private getDurationEstimate(length: QuestLength): string {
    switch (length) {
      ...
      case 'Long': return '5-8 hours';
      case 'Campaign': return '10+ hours';
      default: return '3-4 hours';
    }
  }
  ...
  private generateQuestTitle(type: QuestType, mainElement: string): string {
    ...
    const suffix = Math.random() > 0.5 ? ' ' + this.randomChoice(TITLE_SUFFIXES) : '';
    
    return `${prefix} the ${mainElement}${suffix}`;
  }

  private generateDescription(type: QuestType, context: Record<string, string>): string {
    const templates = QUEST_DESCRIPTIONS[type] || ['A quest awaits those brave enough to accept it.'];
    let description = this.randomChoice(templates);
    
    // Replace placeholders with context
    Object.keys(context).forEach(key => {
      description = description.replace(new RegExp(`{${key}}`, 'g'), context[key]);
    });
    
    return description;
  }

  private generateRewards(difficulty: Difficulty, length: QuestLength): string[] {
  ...
  private generateComplications(includeComplications: boolean, difficulty: Difficulty): string[] {
    if (!includeComplications) return [];
    
    const numComplications = difficulty === 'Epic' ? 2 : difficulty === 'Hard' ? 1 : Math.random() > 0.5 ? 1 : 0;
    const complications = [];
  ...
  private generateBasicQuest(type: QuestType, difficulty: Difficulty, length: QuestLength, params: QuestGenerationParams): Quest {
    const id = crypto.randomUUID();
    const createdAt = new Date();
    const estimatedDuration = this.getDurationEstimate(length);
    const rewards = this.generateRewards(difficulty, length);
    const complications = this.generateComplications(params.includeComplications || false, difficulty);
    ...
    switch (type) {
      case 'Kill Quest': {
        ...
        const description = this.generateDescription(type, { creature: creature.toLowerCase() });
  ...
  private generateAdvancedQuest(type: QuestType, difficulty: Difficulty, length: QuestLength, params: QuestGenerationParams): Quest {
    const id = crypto.randomUUID();
    const createdAt = new Date();
    const estimatedDuration = this.getDurationEstimate(length);
    const rewards = this.generateRewards(difficulty, length);
  ...
  generateQuest(params: QuestGenerationParams = {}): Quest {
    ...
    const questType = params.questType === 'random' || !params.questType 
      ? this.randomChoice([...QUEST_TYPES.basic, ...QUEST_TYPES.advanced])
      : params.questType;

    // Determine difficulty
    const difficulty = params.difficulty === 'random' || !params.difficulty
      ? this.randomChoice(DIFFICULTIES)
      : params.difficulty;

    // Determine length
    const length = params.length === 'random' || !params.length
  ...
  generateMultipleQuests(count: number, params: QuestGenerationParams = {}): Quest[] {
    const quests: Quest[] = [];
